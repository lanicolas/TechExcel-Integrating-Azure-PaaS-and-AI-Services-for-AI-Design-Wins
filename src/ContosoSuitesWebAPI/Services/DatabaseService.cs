...
public class DatabaseService(string connectionString) : IDatabaseService
{
    ...
    [KernelFunction]
    [Description("Get bookings who are missing a Hotel Room.")]
    ...
    [KernelFunction]
    [Description("Get bookings with the number of Guests.")]
    public async Task<IEnumerable<Booking>> GetBookingsWithHotelRoomCount([Description("The ID of the hotel")] int hotelId, [Description("The number of guests per room")] int roomCount)
    {
        var sql = """
            SELECT
                b.BookingID,
